...
| **18** Generation pipeline hardening — two-pass + zod + retry ladder + sensitive-news rule | ✅ | **Merged + deployed to production** via PR #41. 49-assertion smoke test + full-suite regression green; live two-pass generation verified for ALL THREE editions (repair retry and mystery reserve fallback both fired and recovered live). First production morning on the new pipeline is the next 7 AM run. See the Phase 18 session entry. |
