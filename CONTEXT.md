...
| `sw.js` / `pwa.js` | Service worker + add-to-homescreen UX + push (Phase 15). `sw.js`: push handler renders `{title, body, url, tag}` payloads (distinct tags `mj-morning`/`mj-streak` so the kinds don't collapse onto each other); cache prefix `mj-`, version **v5** (Phase 16 added `games/mystery-mover.js` to the shell). `pwa.js`: fetches the VAPID key from `/api/push/public-key`; the permission ASK is a soft banner gated on the kid's **3rd active day** (`progress.activeDays` from the `mj:state-loaded` event engagement.js fires) — never the native prompt on load, never on first visit, never the same visit as the install banner (install takes priority). Silent re-subscribe on load when permission is already granted. iOS requires standalone; Android Chrome subscribes in-tab. |
